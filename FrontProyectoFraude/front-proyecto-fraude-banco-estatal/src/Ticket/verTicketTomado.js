...
function VerTicketTomado() {
    ...
    async function terminar() {
        await fetch("http://localhost:8080/tickets/" + ticketId + "/?status=pending", {
            method: 'POST',
            headers: {
                'Authorization': localStorage.getItem('authorization'),
                "Content-Type": 'application/json',
    ...
    return (
        <div>
            ...
            <br></br>
            <br></br>
            <center><button onClick={terminar}>Terminar</button><button onClick={volver}>Volver</button></center>
        </div>
    );
};

export default VerTicketTomado;